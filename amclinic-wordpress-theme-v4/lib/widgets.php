...
/**
 * Check out our Specialist Clinics widget.
 *
 * @since 0.1.3
 */
class check_out_our_specialist_clinics extends WP_Widget
{
   ...
   function __construct()
   {
	   parent::__construct(
		   ...
		   // Base ID of your widget
		   'check_out_our_specialist_clinics',

		   // Widget name will appear in UI
		   __('Widget Blocks Widget', 'wpb_widget_domain'),
   ...
   public function widget($args, $instance)
   {
	   ...
	   if (!empty($title))
		   echo $args['before_title'] . $title . $args['after_title'];
	   // This is where you run the code and display the output
	   echo do_shortcode('[check_specialist_conditions]');
	   echo $args['after_widget'];
   }
   ...
   public function form($instance)
   {
	   if (isset($instance['title'])) {
		   ...
		   $title = __('Check out our Specialist Clinics', 'wpb_widget_domain');
	   }
	   // Widget admin form
   ?>
	   <p>
...
function wpb_load_widget()
{
	...
	register_widget('check_out_our_specialist_clinics');
}
